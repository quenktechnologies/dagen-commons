...
class ValidationPlugin extends plugin_1.AbstractPlugin {
    ...
    configureGenerator(g) {
        ...
        g.env.addGlobal('hasValidators', (s) => (s[this.key] != null));
        g.env.addGlobal('hasCompleteChecks', (s) => s[this.key] && s[this.key]['complete']);
        g.env.addGlobal('hasCompleteChecks', (s) => s[this.key] && s[this.key]['partial']);
        g.env.addGlobal('getTest', (s) => {
            console.warn('getTest: is deprecated use getValidators instead.');
            return s[this.key];
        });
        g.env.addGlobal('getValidators', (s) => s[this.key]);
        g.env.addGlobal('getCompleteChecks', (s) => s[this.key]['complete']);
        g.env.addGlobal('getPartialChecks', (s) => s[this.key]['partial']);
 ...
 */
exports.create = (ctx) => new ValidationPlugin('validation', 'validation', ctx);
